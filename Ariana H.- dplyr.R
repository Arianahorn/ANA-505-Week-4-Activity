...
total=mutate(newlooks, total=cumsum(n))  
total=mutate(looks, total=cumsum(Freq))  

#Since we have a males dataset, let's make a females dataset
#TASK: Write the function that includes only rows that are 'female'
newlooks %>% filter(Gender == "Female")
female <- newlooks %>% filter(Gender == "Female")
...
newlooks %>% arrange(Eye, Gender)
newlooks %>% arrange(desc(Eye))
